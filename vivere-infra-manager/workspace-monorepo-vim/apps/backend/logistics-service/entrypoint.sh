#!/bin/sh
# =====================================================
# VIVERE PLATFORM – LOGISTICS SERVICE (ENTRYPOINT REVISADO)
# =====================================================
set -e

echo "=================================================="
echo "🚀 Inicializando: ${SERVICE_NAME:-logistics-service}"
echo "🌎 Ambiente: ${NODE_ENV:-development}"
echo "🗄  Banco: ${DATABASE_HOST:-logistics-db}:${DATABASE_PORT:-5432}"
echo "=================================================="

: "${PORT:=3000}"
: "${DB_WAIT_TIMEOUT:=60}"
: "${DATABASE_HOST:=logistics-db}"
: "${DATABASE_PORT:=5432}"
...
if [ "${RUN_POST_SQL:-false}" = "true" ]; then
    POST_SCRIPT="./prisma/logistics_db_post_prisma.sql"
...
echo "🔥 Iniciando Logistics Service (node dist/main.js)..."
