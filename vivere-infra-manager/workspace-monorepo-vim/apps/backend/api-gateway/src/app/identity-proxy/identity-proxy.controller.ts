/**
 * Controller proxy para o Identity Service
 * Endpoint: /identity/*
 */
...
export class IdentityProxyController {
  ...
  constructor(
    ...
    this.baseUrl = this.configService.get<string>('IDENTITY_SERVICE_URL') ?? '';
    if (!this.baseUrl) {
      throw new Error('IDENTITY_SERVICE_URL não definida');
    }
  }
  ...
  async proxy(@Req() req: Request, @Res() res: Response) {
    ...
    const url = `${this.baseUrl}${originalUrl.replace('/api/identity', '')}`;
    const headers = { ...req.headers };
    delete headers.host;
    delete headers['content-length'];

    this.logger.log(`[IDENTITY] ${method} ${url}`);
      ...
      if (error instanceof AxiosError) {
        ...
        if (error.response) {
          ...
          res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
            ...
            message: 'Erro ao comunicar com o Identity Service',
          });
        }
      } else {
