#!/bin/bash
# =====================================================
# VIVERE PLATFORM – WAREHOUSE DATABASE
...
if [ -n "$SEED_ENV" ] && [ -f "/seeds/$SEED_ENV.sql" ]; then
    echo ">>> [WAREHOUSE] Executando seed para ambiente: $SEED_ENV"
    psql -v ON_ERROR_STOP=1 \
         --username "$POSTGRES_USER" \
         --dbname "$POSTGRES_DB" \
         -f "/seeds/$SEED_ENV.sql"
    echo ">>> [WAREHOUSE] Seed concluído com sucesso."
else
    echo ">>> [WAREHOUSE] Nenhum seed encontrado para o ambiente '$SEED_ENV' ou variável não definida."
fi