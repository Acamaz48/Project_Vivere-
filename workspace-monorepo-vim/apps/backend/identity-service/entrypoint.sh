#!/bin/sh
# =====================================================
# VIVERE PLATFORM – IDENTITY SERVICE (ENTRYPOINT REVISADO)
# =====================================================
set -e

echo "=================================================="
echo "🚀 Inicializando: ${SERVICE_NAME:-identity-service}"
echo "🌎 Ambiente: ${NODE_ENV:-development}"
echo "🗄  Banco: ${DATABASE_HOST:-identity-db}:${DATABASE_PORT:-5432}"
echo "=================================================="

: "${PORT:=3000}"
: "${DB_WAIT_TIMEOUT:=60}"
: "${DATABASE_HOST:=identity-db}"
: "${DATABASE_PORT:=5432}"
...
if [ "${RUN_POST_SQL:-false}" = "true" ]; then
    POST_SCRIPT="./prisma/identity_db_post_prisma.sql"
...
echo "🔥 Iniciando Identity Service (node dist/main.js)..."
